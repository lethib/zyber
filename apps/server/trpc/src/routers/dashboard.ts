...
export const dashboardRouter = router({
	getScore: protectedProcedure.input(levelInput).query(async ({ ctx, input }) => {
		return await new OrganizationScore(ctx.organizationId).getScore(toPrismaLevel(input?.level));
	}),

	getThemeBreakdown: protectedProcedure.input(levelInput).query(async ({ ctx, input }) => {
		return await new OrganizationScore(ctx.organizationId).getThemeBreakdown(
			toPrismaLevel(input?.level),
		);
	}),
});
