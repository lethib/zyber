...
import { LEVELS, type Level, ThemeRadarChart } from "./components/ThemeRadarChart";

const dashboardSchema = z.object({
	level: z.enum(LEVELS).default("all"),
});
...
function StatCard({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
	return (
		<Card className="p-4">
			...
			<p
				className={`text-3xl font-semibold tabular-nums ${accent ? "text-green-600" : "text-zinc-700"}`}
			>
				{value}
			</p>
		</Card>
...
function DashboardPage() {
	...
	const queryInput = level !== "all" ? { level: level as "standard" | "enhanced" } : undefined;

	const { data: scoreData, isLoading: scoreLoading } = trpc.dashboard.getScore.useQuery(queryInput);
	const { data: breakdown, isLoading: breakdownLoading } =
		trpc.dashboard.getThemeBreakdown.useQuery(queryInput);
	...
	const gaugeValues = (() => {
		if (!scoreData || !breakdown)
			return { percentage: 0, achieved: 0, total: 0, label: "mesures atteintes" };
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
				...
				<div className="flex flex-col gap-4">
					...
					<Card className="p-6">
						...
						<div className="flex flex-col items-center gap-3">
							{isLoading ? (
								...
								<ScoreGauge score={gaugeValues.percentage} />
							)}
							{isLoading ? (
								<Skeleton className="h-4 w-40 rounded" />
							) : (
								<p className="text-sm text-zinc-500 text-center">
									<span className="font-semibold text-zinc-900">{gaugeValues.achieved}</span>
					...
					{isLoading ? (
						...
						<div className="grid grid-cols-2 gap-3">
							<StatCard label="Atteintes" value={gaugeValues.achieved} accent />
							<StatCard label="Restantes" value={gaugeValues.total - gaugeValues.achieved} />
						</div>
					)}
						...
						<Card className="p-4">
							...
							<div className="w-full h-2 rounded-full bg-zinc-100 overflow-hidden">
								<div
									...
									style={{ width: `${gaugeValues.percentage}%` }}
								/>
							</div>
				...
				<Card className="lg:col-span-2 p-6">
					...
					{isLoading || !breakdown ? (
						...
						<ThemeRadarChart data={breakdown} />
					)}
				</Card>
			</div>
		</div>
